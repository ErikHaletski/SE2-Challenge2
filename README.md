...
- route registration is done directly in `main()`
- filesystem access uses `os.ReadFile`
- file path resolution uses `filepath.Join`
- missing files currently trigger `panic(err)` rather than a graceful HTTP error response

Suggested backend improvements:

- return `404` instead of panicking on missing files
- validate filenames more strictly
- align backend data source with the current Markdown-based content model
- add tests for route behavior
- move configuration such as port/path to environment variables
...
- The project contains a generated `public/` directory; decide whether you want to version generated output long-term.
- The repository contains `.idea/` files; these are IDE-specific.
- CMS-related configuration appears in multiple places (`config.json` and `content/*/admin/config.json`). Keep them synchronized.
- The backend path expectations and the actual content layout are currently not fully aligned.

## Testing Checklist

A practical manual test pass should include:

- homepage renders in both languages
- search page accepts product queries
- fuzzy search returns suggestions for misspellings
- category filtering works
- detail pages display image, labels, and nutrition values
- comparison table updates when opening a product via search results
- `/ping` returns `pong`
- `/produkte/:file` returns valid JSON if JSON files are provided
